...
export default function Search() {
    ...
    const ImageUri = async () => {
        ...
        const uploadTasks = (Data.Photo)?.map(async (item) => {
            ...
            const ref = firebase.storage().ref().child('DuplexForRent/' + (filename[0] + name).toString());
    ...
    const AdToAdmin = async () => {
        const UserRef = doc(db, "DuplexForRentAdmin", '0');
        const docSnap = await getDoc(UserRef);
        ...
        if (docSnap.data() != undefined) {
            ...
            if (Deleted.length == 0) {
                ...
                const UserRef2 = doc(db, "DuplexForRentAdmin", num.toString());
                ...
                const UserRef2 = doc(db, "DuplexForRentAdmin", (val[0]).toString());
                await setDoc(UserRef2, {
                    uid: user.uid,
                    phone: Phone,
                    Photo: arr,
