...
export default function Search() {
    ...
    const ImageUri = async () => {
        ...
        const uploadTasks = (Data.Photo)?.map(async (item) => {
            ...
            const ref = firebase.storage().ref().child('BuildingsLands/' + (filename[0] + name).toString());
    ...
    const AdToAdmin = async () => {
        const UserRef = doc(db, "BuildingsLandsAdmin", '0');
        const docSnap = await getDoc(UserRef);
        ...
        if (docSnap.data() != undefined) {
            ...
            if (Deleted.length == 0) {
                ...
                const UserRef2 = doc(db, "BuildingsLandsAdmin", num.toString());
                ...
                const UserRef2 = doc(db, "BuildingsLandsAdmin", (val[0]).toString());
                await setDoc(UserRef2, {
                    uid: user.uid,
                    phone: Phone,
                    Photo: arr,
            ...
            <View style={{ flex: 1, alignItems: 'center' }}>
                <ScrollView style={{ width: "100%", marginTop: 20 }}>
                    ...
                    <View style={{ width: "100%", alignItems: 'center', marginTop: 10 }}>
                        ...
                        <View style={{ width: "90%", marginBottom: 5, marginTop: 10 }}>
                            <Text style={{ fontSize: 15, fontWeight: 'bold' }}>
                                {Languages.SaleRent} : {Languages[Data.SaleRent]}
                            </Text>
                        </View>
                    </View>
                    <View style={{ width: "100%", alignItems: 'center', marginTop: 10 }}>
